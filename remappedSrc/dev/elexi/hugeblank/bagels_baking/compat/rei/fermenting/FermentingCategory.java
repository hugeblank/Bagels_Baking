...
public class FermentingCategory implements DisplayCategory<FermentingDisplay> {
    ...
    public @NotNull Renderer getIcon() {
        return EntryStacks.of(Baking.FERMENTER_ITEM);
    }

    @Override
    public Text getTitle() {
        return new TranslatableText("category.rei.fermenting");
    }

    @Override
    public @NotNull List<Widget> setupDisplay(FermentingDisplay display, Rectangle bounds) {
        Point startPoint = new Point(bounds.getCenterX() - 41, bounds.getCenterY() - 20);
        List<Widget> widgets = Lists.newArrayList();
        widgets.add(Widgets.createRecipeBase(bounds));
        widgets.add(Widgets.createArrow(new Point(startPoint.x + 27, startPoint.y + 4)));
        widgets.add(Widgets.createSlot(new Point(startPoint.x + 30, startPoint.y + 23)).entries(display.getCollectorEntries().get(0)).markInput());
